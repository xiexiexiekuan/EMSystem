...
public class AdminController {
    ...
    public String examType(Map<String,Object> map){
        List<ExamType> examType = adminService.findAllExamType();
    ...
    public String toExamTypeUpdate(int typeId, Map<String,Object> map){
        ExamType updateType = adminService.findExamTypeById(typeId);
    ...
    public String examTypeUpdate(Map<String,Object> map, ExamType examType){
        Integer i = adminService.updateExamType(examType);
        if(i>0){
    ...
    public String examTypeDelete(int typeId, Map<String,Object> map){
        Integer i = adminService.deleteExamType(typeId);
        if(i>0){
    ...
    public String publishExam(PublishExam exam, Map<String,Object> map){
        ...
        Integer h = adminService.findPublishExamByTypeId(exam.getTypeId());//同一种类型考试只能发布一次
        if(h!=null){//如果没有查询到结果，会返回一个空指针给h
            map.put("msg","本类型考试已经发布！");
            return toPublishExam();
        }
        Integer i = adminService.publishExam(exam);
        if(i>0){
    ...
    public String examInfo(Map<String,Object> map){
        List<PublishExam> publichExam = adminService.findAllPublishExam();
    ...
    public String toTestPaperPlan(Map<String,Object> map){
        List<JudgingPlan> judgingPlan = adminService.findAllJudgingPlan();
    ...
    public String toTestPaperPlanUpdate(int planCode, Map<String,Object> map){
        JudgingPlan updatePlan = adminService.findJudgingPlanByCode(planCode);
    ...
    public String testPaperPlanAdd(Map<String,Object> map, JudgingPlan judgingPlan){
        Integer i = adminService.addJudgingPlan(judgingPlan);
        if(i>0){
    ...
    public String testPaperPlanUpdate(Map<String,Object> map, JudgingPlan judgingPlan){
        Integer i = adminService.updateJudgingPlan(judgingPlan);
        if(i>0){
    ...
    public String testPaperPlanDelete(int planCode, Map<String,Object> map){
        Integer i = adminService.deleteJudgingPlan(planCode);
        if(i>0){
    ...
    public String violateCode(Map<String,Object> map){
        List<ViolationsCode> violationsCode = adminService.findAllViolationsCode();
    ...
    public String punishCode(Map<String,Object> map){
        List<PenaltyCode> penaltyCode = adminService.findAllPenaltyCode();
    ...
    public String violateRecords(Map<String,Object> map){
        List<ViolationInfo> violationInfo = adminService.findAllViolationInfo();
    ...
    public String violateHandle(ViolationInfo violationInfo, Map<String,Object> map){
        Integer i = adminService.handleViolationInfo(violationInfo);
        if(i>0){
    ...
    {
        List<ApplicationInformation> gradesInfo = adminService.findAllGradesInfo();
        for(int i=0; i<gradesInfo.size(); i++){
            Integer isViolate = adminService.findViolationInfoByUserId(gradesInfo.get(i).getUserId());//依次查找每个学生是否有违规记录
            if(isViolate==null) gradesInfo.get(i).setPayStatus(0);//ExamStatus原为开考状态，在这里用为是否违规
            else gradesInfo.get(i).setPayStatus(1);
        }
        List<ExamType> examMenu = adminService.findExamMenu();
    ...
    {
        List<ApplicationInformation> gradesInfo = adminService.findOneGradesInfo(typeId);
        for(int i=0; i<gradesInfo.size(); i++){
            int isViolate = adminService.findViolationInfoByUserId(gradesInfo.get(i).getUserId());//依次查找每个学生是否有违规记录
    ...
    {
        Integer i = adminService.gradesManage(enterId, grades);
        if(i>0){
